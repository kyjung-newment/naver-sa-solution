...
### 핵심 로직
- **블렌딩 ROAS** = 주차별 분리 구간 가중 합산 (1주차 40% / 2주차 30% / 3~4주차 30%)
...
- 모든 파라미터·규칙은 설정 화면에서 상시 수정 가능 (변경 이력 `bid_setting_history` 기록)
...
- **월간**: 매월 1일 06:00 — 4주 연속 증액/감액 소재 목표ROAS 재검토 리포트, 수동 ±15%

### 안전장치
- 변경 폭 ±20% 초과 건은 auto 모드여도 무조건 승인 필요
- 적용 전 현재 입찰가 API 재조회 검증 (불일치 시 경고 기록)
- 실패 건 `failed` 상태 + 승인함에서 재시도 · 모든 변경 `bid_audit_log` 기록 (삭제 불가)

### 권한
- **운영자(admin)** = 기존 마케터 계정 그대로 로그인 (전체 설정/실행)
- **광고주(client)** = `/egojin-bid/viewers` 에서 이메일 초대 → 초대 수락 계정은 조회 + 조정 승인/반려만
  가능 (기존 `account_viewers` 초대 구조 재사용, 초대 메일은 운영자 다우오피스 SMTP 발송)
