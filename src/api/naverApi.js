...
/**
 * 네이버 광고관리 내부 API를 통한 성별/연령 데이터 조회
 * (cookie-based auth, ads.naver.com)
 * @param {string} customerId
 * @param {string} cookie - 네이버 로그인 세션 쿠키
 * @param {{since:string, until:string}} dateRange
 * @param {'gender'|'age'} dimension
 * @returns {Array<{label:string, imp:number, clk:number, cost:number, convCnt:number, convAmt:number}>}
 */
async function fetchDemographicData(customerId, cookie, dateRange, dimension) {
  const attribute = dimension === 'gender' ? 'criterionGenderNm' : 'criterionAgeTpNm';

  try {
    const resp = await axios({
      method: 'POST',
      url: 'https://ads.naver.com/apis/sa/api/advanced-report/values',
      headers: {
      ...
      data: {
        ...
        metrics: ['impCnt', 'clkCnt', 'salesAmt', 'ccnt', 'convAmt'],
    ...
    const raw = resp.data;
    console.log(`📊 Demographic API (${dimension}) 응답:`, JSON.stringify(raw).slice(0, 500));
    ...
    return rows.map(r => ({
      label: r[attribute] || r.label || r.name || '알수없음',
      imp: Number(r.impCnt || r.imp || 0),
      clk: Number(r.clkCnt || r.clk || 0),
      cost: Number(r.salesAmt || r.cost || 0),
      convCnt: Number(r.ccnt || r.convCnt || 0),
      convAmt: Number(r.convAmt || 0),
    }));
  } catch (err) {
    const status = err.response?.status;
    if (status === 401 || status === 403) {
      throw new Error('네이버 쿠키가 만료되었습니다. 광고주 설정에서 쿠키를 갱신해주세요.');
    }
    console.error(`❌ Demographic API (${dimension}) 실패 [${status}]:`, err.response?.data || err.message);
    throw new Error(`성별/연령 API 호출 실패: ${err.message}`);
  }
}

module.exports = { createApiClient, fetchDemographicData };
